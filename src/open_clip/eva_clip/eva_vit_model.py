...
class EVAVisionTransformer(nn.Module):
    ...
    def encode_dense(self, x, keep_shape=True):
        ...
        for blk in self.blocks[:-1]:
            # print(x.shape) torch.Size([2, 4097, 768])
            x = blk(x, rel_pos_bias=rel_pos_bias)

        x = self.blocks[-1].forward_without_attn(x)[:, 1:] # 转到 src/open_clip/eva_clip/eva_vit_model.py 的 forward_without_attn
        x_map = x
        x = self.norm(x)
        x = self.head(x)
    ...
    def extract_roi_features(self, x, normed_boxes, **kwargs):
        x, x_map = self.encode_dense(x, keep_shape=True)
        return roi_align(x, self._denormalize_boxes(normed_boxes, x), (1, 1),
                         1.0, -1, True)[..., 0, 0], x_map
    ...
    def mask_pool(self, x, masks):
        feature_map, _ = self.encode_dense(x, keep_shape=False)
