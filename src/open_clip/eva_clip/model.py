...
class CustomCLIP(nn.Module):
    ...
    def encode_dense(self, image, normalize: bool = False, keep_shape=False):
        features, _ = self.visual.encode_dense(image, keep_shape=keep_shape)
        if normalize:
            if keep_shape:
    ...
    def encode_pseudo_boxes(self, image, normed_boxes, normalize: bool = False,
                            extract_type='v1'):
        # 转到 src/open_clip/eva_clip/eva_vit_model.py 的 extract_roi_features
        features, feature_maps = self.visual.extract_roi_features(image, normed_boxes, extract_type=extract_type)
        if normalize:
            features = F.normalize(features, dim=-1)
